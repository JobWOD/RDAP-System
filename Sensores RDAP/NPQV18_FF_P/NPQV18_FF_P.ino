...
void setup(){   
  // initialize timer1 
  val = r = p = i = inmode = concam = conmp = 0;
...
{      
  ...
  switch (sel){
    ...
    case 6:  
      ...
      lcd.setCursor ( 0, 1 );
      lcd.print("Press.: ");
      lcd.print(rcap);      // rcap     //Variable de presion   //envio
      lcd.print(" MCA");
      break;           
    case 7:  
      lcd.setCursor ( 0, 0 );
  ...
  if (x_bee.available() > 0 ){   // si es mayor a 0 se recibio un dato  
    ...
    if(a == 67 || a == 99){    // C en ascii
      x_bee.print("CFP");     // Se envia la cadena de reconocimiento de sensores
    }
